...
#pragma warning restore CS0618 // Typ oder Element ist veraltet
            ...
            // CheckInfo(dataCollection, _file.Info, afterHeaderPosition);
        }

        private void CheckInfo(DataCollection dataCollection, CR2WFileInfo cr2WFileInfo, int afterHeaderPosition)
        {
            for (int i = 0; i < dataCollection.ChunkInfoList.Count; i++)
            {
                var newInfo = dataCollection.ChunkInfoList[i];
                var oldInfo = cr2WFileInfo.ExportInfo[i];
            ...
            for (int i = 0; i < dataCollection.BufferInfoList.Count; i++)
            {
                ...
                var oldInfo = cr2WFileInfo.BufferInfo[i];
